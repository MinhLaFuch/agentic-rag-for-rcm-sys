...
# API related info
# This script intentionally uses the values defined here for the run.
API_KEY="sk-351feed00563b403c014df324c1ff126bfa35973b6885fde7ccc67b31715300d" # your api key
API_BASE="https://api.xah.io/v1" # [https://xxxx.azure.com, https://api.openai.com/v1]
API_VERSION=""
API_TYPE="open_ai" # ['open_ai', 'azure']
engine="gpt-5.4"   # model name for OpenAI or deployment name for Azure OpenAI. GPT-4 is recommended.
...
# This launcher intentionally uses the API settings defined in this file and
# does not read from the local shell environment.
...
export OPENAI_API_TYPE="$API_TYPE"
export TOKENIZERS_PARALLELISM=false
export PYTHONPATH="$(pwd)"

python ./app.py --engine="$engine" --bot_type="$bot_type" --enable_shorten="$enable_shorten" --demo_mode="$demo_mode" --num_demos="$num_demos" --enable_reflection="$enable_reflection" --plan_first="$plan_first" --langchain="$langchain" --demo_dir_or_file="$demo_dir_or_file"
