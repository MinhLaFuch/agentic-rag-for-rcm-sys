...
from package.data.utils import load_reviews_and_metadata
...
def test_load_reviews_dataframe_reads_all_rows(synthetic_review_file):
    df = load_reviews_and_metadata(synthetic_review_file)
    assert len(df) == 4
...
def test_load_reviews_dataframe_with_column_subset(synthetic_review_file):
    df = load_reviews_and_metadata(synthetic_review_file, columns=["user_id", "parent_asin"])
...
def test_load_reviews_dataframe_missing_column_raises(synthetic_review_file):
    with pytest.raises(ValueError):
        load_reviews_and_metadata(synthetic_review_file, columns=["not_a_real_column"])
